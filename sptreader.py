...
def toIntelHex(spt_file):
  """
    ...
    Return value:
      ...
      Separation between 2 entries in the list is disctated by the presence of
      an explicit write to 1 at CPUCS address in input file.
  """
  ihx_list = []
  while True:
    memory = {}
    ...
    if len(memory):
      ...
      for offset in offset_list:
        if offset < next_offset:
          raise ValueError, 'Code overwriting itself: 0x%x < 0x%x' % (offset,
            next_offset)
        data = memory[offset]
        next_offset = offset
        while data:
          chunk, data = data[:IHX_MAX_LINE_LENGTH], data[IHX_MAX_LINE_LENGTH:]
          chunk_len = len(chunk)
          line = [chunk_len, (next_offset >> 8) & 0xff, next_offset & 0xff,
            0] + [ord(x) for x in chunk]
